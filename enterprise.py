"""
Enterprise controls for ComplianceRAG — the showcase differentiator.
...
interview: each function is small enough to read aloud and defend. This is a
standalone copy — ComplianceRAG imports nothing from sibling repos.

Usage (wired into compliancerag.py):
...
# Meta note: OWASP LLM01 is itself one of the frameworks this app cites — the
# guardrail defends the assistant that explains the guardrail.
...
def guard_output(answer: str, source_names: list[str]) -> tuple[str, bool]:
    ...
    For a compliance assistant this is the whole point: an uncited claim about
    "what the framework says" is a liability, not an answer.
